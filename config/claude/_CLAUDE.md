...
- **バックグラウンドエージェントの完了を厳守すること**: `run_in_background: true` でエージェントを起動した場合:
    - そのエージェントの `<task-notification>` (完了通知) が届くまで、担当トピックについて結論を出したり次のステップに進んではならない
    - 完了通知が届いていないエージェントの結果を「まとめ」たり「報告」したりしてはならない。推測や仮の情報で進めることは禁止
    - 複数のバックグラウンドエージェントを起動した場合、すべての完了通知が届いてから次のステップに進むこと
    - ユーザーへの中間報告では、完了したものと未完了のものを明確に区別すること
    - エージェントに委任した作業を自分で重複実行するのは無駄であり禁止
    - **完了の自己判断禁止**: エージェントの完了はシステムからの `<task-notification>` タグでのみ判断すること。自分で `[agent completed]` 等のテキストを生成してはならない。出力ファイルを途中で Read して「実質的に結果が得られた」と判断して先に進むのも禁止
...
- 外部リポジトリ: 分析用に利用可能にするには `ghq get -u <org>/<repo-name>` を使用してクローン (既に存在する場合は更新)
