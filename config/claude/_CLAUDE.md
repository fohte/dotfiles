...
- **qmd の優先使用**: ファイルの読み取りやディレクトリの探索を行う前に、まず qmd MCP サーバーでインデックスを検索すること
- 利用可能なツール:
    - `qmd_search "query"`: BM25 キーワード検索 (高速。80% のケースはこれで十分)
    - `qmd_query "query"`: ハイブリッド検索 + リランキング (最高品質。複雑な質問向け)
    - `qmd_vsearch "query"`: セマンティックベクトル検索 (キーワードが不明確なとき)
    - `qmd_get <file>`: 特定ドキュメントの取得
- qmd で十分な結果が得られない場合にのみ、Read/Glob/Grep にフォールバックすること
