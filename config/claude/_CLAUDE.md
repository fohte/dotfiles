...
- **qmd の優先使用 (検索時)**: ファイルの内容やキーワードを**検索**する場合 (何がどこにあるか探す、関連コードを見つけるなど) は、Grep/Glob の前にまず qmd MCP サーバーの `search` を使用すること
    - qmd で十分な結果が得られない場合にのみ、Grep/Glob にフォールバックする
- **パスが明示的にわかっている場合**: ファイルパスが既知の場合は qmd の `get`/`multi_get` または Read を直接使用してよい (検索を経由する必要はない)
