...
- **サブエージェント**: 単一ファイルの読み書きのような些細なタスクを除き、専門サブエージェントへの委任をデフォルトとする。独立したタスクは MECE に分解して複数エージェントを並列起動する
    - Claude Code: Task ツール。単一メッセージ内のツール呼び出しもバッチで並列実行する
...
- **例には架空の値を使う**: コード、fixture、図、コメント、commit message、PR description、ドキュメントで例を書くとき、作業中に目にした実データ (出所を問わない) の固有名詞や具体的な値を使わない。成果物が private でも同じ
...
- コメント
    ...
    - 複数の事実をまとめた解説ブロックを作らず、各事実をそれが説明する対象 (読み手がそこで疑問を持つ箇所) の直近に分散して置く
