...
- 他の Claude Code セッションを探すとき: `a agent peer list` (`parent` / `children` は絞り込み版)。出力の `name` をそのまま `SendMessage` の `to` に渡す。`name` が `null` なら停止中なので、用件が確定しているときだけ `a agent peer wake <session_id>` で起こす
