...
- **専門サブエージェントの必須使用**: その専門知識に合致するタスクについては、**必ず常に**適切なサブエージェントと共に Task ツールを使用すること。タスクが些細なもの（単一ファイルの読み取り/編集）でない限り、サブエージェントの使用をデフォルトとすること。独立したタスクについては、複数のエージェントを並列で起動すること- **バックグラウンドエージェントの結果を待つこと**: `run_in_background: true` でエージェントを起動した場合、そのエージェントが担当するトピックについて自分で調査を開始してはならない。完了通知が届くまで待ち、結果を使って次のステップに進むこと。エージェントに委任した作業を自分で重複実行するのは無駄であり禁止
