...
3. `comment.md` の判定手順に従い、追加・変更されたコメント 1 件ごとに、まず理想形を書いてから理想形にテスト 1-4 を、書かれたコメントにテスト 5-6 をかける。コメント単体で判断せず、必ず対象コードを Read して「そのコメントが無い状態でコードを読んだら誤解するか」「コメントの主張がコードと一致しているか」を裏取りする。
