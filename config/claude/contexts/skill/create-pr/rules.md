...
静的ファイル追加・設定変更・機械的変更のような小さな PR は Purpose / Approach 各 1 行を既定とし、Design decisions は省略する。全セクションを「埋めにいく」と必ず過剰になる。「この変更規模に対して body が長すぎないか」を常に自問し、長ければ各セクションを 1 行まで削る。
