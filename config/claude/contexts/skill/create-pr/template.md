...
2. **Step 1 で示した全ルールを順に**、各ルールについて以下の形式でユーザーに発話する{{ if $has_design_decisions }} (Rule 1〜13){{ else }} (Rule 1〜8, 10〜13。Rule 9 は省略){{ end }}:
...
3. **減算チェック (独立工程)**: ルール照合後、各行・サブバレット・テーブルセルを 1 つずつ取り上げ、以下 2 つの問いを順に当てる:
    ...
    **警戒シグナル**: 減算チェックで「削る」判定が一個も出なかった場合は基準が「あった方が親切」に緩んでいる兆候。Why/What がそれぞれ 2 項目以上ある場合、通常 1 つは削れる。再度 Q1/Q2 を厳しめに当て直す。数行削っただけで満足して止まらない (一部削除すると警戒シグナルが鳴らず、テーブル丸ごとや冗長な本文が生き残る)。
