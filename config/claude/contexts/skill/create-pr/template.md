...
- **タスク指示文を転記しない**: 委任タスクや issue は実装者向け。実装者向けの注意 (「〇〇は残す」、SHA/pin 値、手順の理由) はレビュアーに不要
- **コード要素はバッククォート**: ファイル名、関数名、変数名、コマンド名

### タイトル

50 文字以内、現在形の動詞 (「Add ...」「Fix ...」)、日本語。**客観的な変更要約**を書き、未確証の結果断定 (「〜が成功するようにする」) は不可。
{{- if $release_please }}

このリポジトリは **release-please を使用している** ため Conventional Commits 形式 (`<type>(<scope>): <description>`) を使う。type 判定は「エンドユーザーに影響するか?」: YES なら新機能 = `feat` / バグ修正 = `fix`、NO なら変更対象で `docs`/`test`/`refactor`/`perf`/`build`/`ci`/`chore`/`style` から選ぶ。

description は動詞形。type との二重表現 (`fix(api): エラーハンドリングを修正`) 不可。
{{- else }}
...
- **書き分け**: バグ修正は「何が問題か」(症状/影響)、新規追加は「何を実現したいか」(目的/動機) を最初の箇条書きで述べる
    - 新規追加で機能/リソースの一行説明だけで終わらせない (「で、なんで今これが必要なの?」が残るなら動機が抜けている)
    - 「〜がない」「〜が存在しない」と書かない (新しいものを作るのだから「ない」のは当然 → 実現したいことを書く)
    - 委任タスクや issue の「背景」「目的」「直前の経緯」セクションは動機そのもの。Purpose に必ず反映する
- **症状/影響だけ書き、技術的原因は書かない** (技術的原因は What の子要素へ)。ただしエラーメッセージ・観測された状態 (status, health, 件数等) は症状の具体化なので残す
- **「〜とき」「〜ので」で文を始めない**。主語と結論を先に。`When ...` `If ...` で始めない
- **解決策を混ぜない**。Why は問題/制約の事実だけ。「△△なら動く」は What に
...
- **外部 org の cross-reference 防止**: PR を出すリポジトリと**異なる org** の URL は `redirect.github.com` 経由 (`github.com` → `redirect.github.com` に置換)。同 org 内 (別リポジトリ含む) は通常通り
- **根拠は引用 + リンクをセット**で記載 (原文ママ、要約・省略禁止):
    ```markdown
    > 引用文
    > [ドキュメント名](URL)
    ```
...
- **Why の回答になるように書く**。Why との繋がりが見えないコマンド・機能の羅列は不可
- **What を書く、How は書かない**: 変更の効果・影響を書く。ファイルパス・行番号・具体的なコード変更は書かない
- **技術的原因は子要素として補足** (バグ修正の場合):
    ```markdown
    - 範囲指定子を含むバージョンを正しく抽出・比較できるようにする
        - 正規表現が `v` プレフィックスのみを考慮しており...
    ```

    - 子要素は**現状の仕組み**の説明だけ。「どういう経緯で今の状態になったか」は書かない
- **実装の内訳を並べない**: diff で見える内訳をコロン区切り・括弧書き・サブバレット・diff/patch 形式で付けない。1 つの変更を構成要素に分解した列挙は全て内訳 (ファイル名、UI パーツ、テストカテゴリ、設定キー、件数、`+`/`-` 行など)
    - ❌ `プロジェクトの雛形を作成する: app.gemspec, Gemfile, Rakefile`
    - ✅ `プロジェクトの雛形を作成する`
- **内部タスク管理用語を書かない**: Phase 名、フェーズ番号、タスク ID 等はレビュアーに伝わらない
- **変更の前後がわかる具体例を示す**: 設定例・コマンド実行例・出力例をコードブロックで。バグ修正は「修正前→修正後」。これは内訳とは別物
- **無理に箇条書きにしない**: 流れで読ませたい時は段落形式を使う
...
- **補足は括弧書きで流さずサブバレットに**: 親項目の文末に `(...)` で補足を詰めない (短い言い換え・単位・略称展開は除く)
    ```markdown
    - 入力をブロック単位で評価する
        - 構文上の特殊ケースは引き続き 1 ブロック扱いのまま保持する
    ```
- **箇条書き 1 項目内に空行を入れない**: 入れると "loose list" 化して `<p>` に囲まれ間隔が広がる。子要素 (引用・コード・サブバレット) は親項目の直後に空行なしでインデント
- 「同じ項目内で段落を分けたい」と感じたら別項目に切り出す合図
...
1. 投入されたファイル (`/tmp/pr-body-draft/<owner>/<repo>/<branch>.md`) を Read し、各行を Step 1 のルールに 1 行ずつ照合
2. **減算チェック (独立工程)**: 各行・サブバレット・テーブルセルを 1 つずつ取り上げ、「この行を消したらレビュアーが判断できなくなるか?」を自問。「あった方が親切」「念のため」「補足として」は全て削除対象
3. 違反・冗長があればファイルを直接編集して修正
   {{- end }}
