...
3. **減算チェック (必須・独立工程)**: ドラフトの各行・各サブバレット・各テーブルセルを 1 つずつ取り上げ、「この行を消したらレビュアーが判断できなくなるか?」を自問する。「あった方が親切」「念のため残す」「補足として書いた」は全て削除対象。判断に必須でない要素はその場で削る。書いた本人は加算的に書きがちなので、ルール違反でなくても本質でない行は残ってしまう (=ルール照合だけでは検出されない)。この工程を省略すると「ながすぎ」の差し戻しを受ける
4. 違反があれば修正してから `a ai pr-draft new` を実行する

「ルールを読んだ = 違反していない」ではない。「ルールに違反していない = 簡潔」でもない。事前 Read したルールは執筆中に頭から抜ける。書いた後にルールファイルを開き直して照合し、さらに減算チェックで本質に絞らない限り、違反 or 冗長を残したまま提出し、ユーザーから差し戻しを受けて結局同じルールを全部再読することになる。
...
- 「〇〇は残す」「△△はスコープ外」という現状維持・スコープ外への言及 (レビュアーは diff で見えないもの = 言及不要)
...
- 既存パターン踏襲の手順説明 (例: 「SSM → helmfile → Secret に流し込む」のような、既存リポジトリで確立されたパターンに従っただけの注入経路): レビュアーは diff を見れば確認できるので Approach に書く必要がない
- 内部タスク管理用語 (Phase 名、フェーズ番号、タスク ID 等): タスク指示文で使われていてもレビュアーには伝わらないので外部にも通じる表現に置き換える
