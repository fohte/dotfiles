...
3. **要素抽出チェック (独立工程)**: 各バレット・テーブルセルから「diff/コード上に現れる固有名」(設定キー名、フィールド名、属性名、関数名、定数名、ファイルパス、正規表現の構造名等) を機械的に抽出する。文章として読むと効果に見える文 (例: `Y で X を限定する`) でも、固有名が抽出されたら手段の言い換えと判定し ❌ にする。Rule 6 の文章レビューでは「文全体の意味が効果寄りだから OK」と通してしまう経路があるため、この独立工程で別軸の検出を入れる。形式:

    ```
    要素抽出: <該当行> → 抽出された固有名 (なし → ✅ / あり → ❌ 削るか書き換える)
    ```
