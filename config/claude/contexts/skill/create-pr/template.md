...
下のコマンドでドラフトを投入したら、**続けて必ず「セルフレビュー」を行うこと**。投入だけで Step 2 (`a ai pr-draft review`) に進んではならない。
{{- if $has_pr_template }}
...
### セルフレビュー (`a ai pr-draft review` 実行前に必ず行う)

ドラフト投入後、`a ai pr-draft review` でユーザーに見せる前に、投入されたファイルに対して以下を行う:

1. **本ファイルを上から読み直す**。頭の中の要約で代替しない。事前に読んだルールは執筆中に頭から抜ける
2. 投入されたファイルの各行を、上の各ルールの ❌/✅ パターンに 1 行ずつ照合
3. **減算チェック (独立工程)**: 各行・サブバレット・テーブルセルを 1 つずつ取り上げ、「この行を消したらレビュアーが判断できなくなるか?」を自問。「あった方が親切」「念のため」「補足として」は全て削除対象。書いた本人は加算的に書きがちなので、ルール照合だけでは検出されない
4. 違反・冗長があればファイルを直接編集して修正してから Step 2 (`a ai pr-draft review`) に進む

「ルールを読んだ = 違反していない」ではない。「違反していない = 簡潔」でもない。投入後に開き直して照合し、減算チェックで本質に絞らない限り、違反・冗長を残したまま提出してユーザーから差し戻しを受ける。
{{- end }}
