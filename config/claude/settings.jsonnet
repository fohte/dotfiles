...
local obsidianVaultPath = env('OBSIDIAN_VAULT_PATH');

{
  ...
  permissions: {
    allow: [
    ...
    ] + (if obsidianVaultPath == '' then [] else [
      ...
      'Edit(' + obsidianVaultPath + '/**)',
      'Edit(' + std.strReplace(obsidianVaultPath, '~/Dropbox', '~/Library/CloudStorage/Dropbox') + '/**)',
    ]) + [
  ...
  // `$defaults` keeps the built-in rules; omitting it replaces them.
  // https://code.claude.com/docs/en/auto-mode-config.md#override-the-block-and-allow-rules
  autoMode: {
    hard_deny: [
      '$defaults',
