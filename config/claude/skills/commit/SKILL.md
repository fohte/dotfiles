...
- 問題: 独立した複数の修正 (例: 起動パフォーマンスの改善とコメントの追記) を 1 コミットにまとめている。subject もレビュー工程を主語にしており、実際に何を直したか読み取れない
...
修正コミットも通常の「コミットの粒度」「コミットメッセージフォーマット」に従う。
指摘が複数ファイル・複数の独立した変更にまたがる場合、1 コミットにまとめず指摘ごと (または関連する 1 まとまりごと) に分割する。
