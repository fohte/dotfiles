...
- **箇条書きに具体物を列挙しない**: What の各項目は変更の意図・効果だけを書く。ファイル名、ツール名、設定値、テストカテゴリ、機能の内訳などをコロン区切り・括弧書き・サブバレットで付け足さない。それらは diff を見ればわかる
    - ❌ `プロジェクトの雛形を作成する: app.gemspec, Gemfile, Rakefile, lib/app.rb` (ファイル名の列挙)
    - ❌ `CI を追加する: v3.1〜4.0 のテストマトリクス、linter による静的解析` (ツール名・設定値の列挙)
    - ❌ `E2E テストを追加する` + サブバレットでテストカテゴリを列挙 (テスト対象の内訳列挙)
    - ✅ `プロジェクトの雛形を作成する`
    - ✅ `CI/CD ワークフローを追加する`
    - ✅ `E2E テストを追加する`
- **具体的な件数・数量を書かない**: テスト件数やファイル数など、diff を見ればわかる数量は記載しない。件数は変更のたびにメンテナンスが必要になるため不適切
