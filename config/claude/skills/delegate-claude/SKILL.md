...
```bash
a wm new <branch-name> --agent --prompt "<instructions>"
```
...
- `--from <ref>`: ベースとなる ref を指定 (デフォルト: main ブランチ)
    - 例: `a wm new feature-x --agent --from origin/develop --prompt "..."`
    - Renovate の PR をテストする場合: `a wm new test-upgrade --agent --from origin/renovate/some-branch --prompt "..."`

### 例

```bash
# 現在のリポジトリ
a wm new feature-login --agent --prompt "メール/パスワード認証によるログイン機能を実装"

# 別のリポジトリ
cd ~/ghq/github.com/fohte/other-repo && a wm new feature-x --agent --prompt "機能 X を実装"
```
...
```bash
a wm new fix-auth-timeout --agent --prompt "## 背景
...
a wm new fix-bug --agent --prompt "さっき話したバグを直して"

# NG: 手順を指示しすぎ - 委任先の自律性を奪う
a wm new fix-auth --agent --prompt "## タスク
