...
- **委任後に完了をポーリングしない**: 完了は委任先から報告が来る (後述の「委任先から委任元に連絡する場合」)。届くのを待てばよく、自作のポーリングで先回りしてはならない。委任が始まったことをユーザーに報告したら、そのターンは終わりにする。委任は委任元セッションを解放するための手段であり、張り付いて見守る対象ではない。ユーザーに訊かれて `a cc peer children` で状態を答えるのは構わないが、訊かれてもいないのに状態を確認して報告するのは、それ自体がポーリングにあたる
...
**作業を終えたら委任元に報告する。** 委任元は完了をポーリングしないので、報告しない限り委任が終わったことがどこにも伝わらない。報告に含めるもの:

- どの委任が終わったのか。委任元は複数の委任を並行させているので、タスク名だけでは判別できないことがある
- 成果物の所在。PR の URL、direct-commit モードなら push したコミット
- ゴールに届かなかったものがあれば、何が残っているか

ゴールに届かないまま作業を続けられなくなったときも、止まる前に同じ手順で報告する。黙って止まると `a cc sweep` にセッションを停止され、報告する機会自体が無くなる。
