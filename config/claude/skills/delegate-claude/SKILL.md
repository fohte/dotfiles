...
- **SendMessage を進捗確認や催促に使わない**: 委任先は別プロセスだが同じマシン上の Claude Code セッションなので、ListAgents に現れ SendMessage も届く。届くからといって、状況を尋ねる、急かす、作業中に細かく口を出すといった用途に使ってはならない。委任先は対話しながら進める相手ではない。送ってよいのは「委任元が渡した前提が誤っていた場合の訂正」だけで、手順は後述の「委任後に前提の誤りが判明した場合」に従う
...
委任は原則として投げっぱなしにする。ただし**委任元が渡した前提そのものが誤っていたと判明した場合だけ**は例外で、訂正を送る。放置すると委任先は誤った前提のまま実装し、その前提がコード中のコメントや PR description に書き込まれて成果物として残る。委任元の誤りを委任先に押し付ける形になるため、気付いた側が訂正する責任を持つ。
...
3. `SendMessage` で訂正を送る
4. 対象が一覧に無い、または `wake` が名前を返さない場合は、誤った前提が既に成果物に入っている。PR が作成済みならその PR にコメントとして訂正を残し、まだ無ければユーザーに報告する

### 訂正に含める内容

- **委任元からの訂正であること**: 委任先は無関係な第三者からの割り込みと区別できないため、最初に明示する
- **何が誤っていたか**: プロンプトのどの記述が誤りかを引用して特定する
