...
`ListAgents` の一覧から探してはならない。停止中のセッションは `ListAgents` に現れないため、生きている委任先を終了したと誤認して、同じタスクを新規に委任し直すことになる。判断の材料にしてよいのは `a cc peer children` の出力だけで、そこに `paused` として載っていれば委任先は失われていない。
...
放置すると委任先は誤った前提のまま実装し、その前提がコード中のコメントや PR description に書き込まれて成果物として残る。委任元の誤りを委任先に押し付ける形になるため、気付いた側が訂正する責任を持つ。
