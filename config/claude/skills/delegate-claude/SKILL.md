...
- **委任後に完了をポーリングしない**: `a cc new` に完了通知の仕組みはないが、それを自作のポーリングで代替してはならない。委任が始まったことをユーザーに報告したら完了とする。委任は委任元セッションを解放するための手段であり、張り付いて見守る対象ではない。ユーザーに訊かれて `a cc peer children` で状態を答えるのは構わないが、訊かれてもいないのに状態を確認して報告するのは、それ自体がポーリングにあたる
