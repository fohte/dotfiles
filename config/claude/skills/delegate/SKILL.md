...
基準は変更行数ではなく、**実装中にどれだけ探索、既存設計の理解、原因調査、判断が要るか**。

| 指定                                            | 使いどころ                                                                             |
| ----------------------------------------------- | -------------------------------------------------------------------------------------- |
| `--model gpt-5.6-sol --reasoning-effort low`    | 既定。方針が決まっていて、既存コードを見ながら一般的な実装を進める                     |
| `--model gpt-5.6-sol --reasoning-effort medium` | 複数コンポーネントにまたがる変更、既存パターンの調査、非自明なテスト修正、軽いデバッグ |
| `--model gpt-5.6-sol --reasoning-effort high`   | 広範囲な探索、原因が明確でないバグ、設計の穴を埋めながらの実装、大規模な変更           |
| `--model gpt-5.6-terra`                         | 変更対象と方法がほぼ決まっていて、探索も判断もほとんど要らない                         |
| `--model gpt-6-astra`                           | Sol High でも解けない原因調査や設計判断                                                |

`--reasoning-effort` を省略した行は既定値をそのまま継ぐので、Terra や Astra で効きを変えたいなら併せて指定する。

`gpt-5.6-luna` は原則使わない。使うのは rename や機械的置換のように判断が要らない作業に限る。**コード生成そのものよりも、リポジトリ探索、skill の選択、状況に応じた方針転換で弱さが出る**ため、変更が小さいことを理由に選ばない。

選ぶときは「安いモデルで足りるか」ではなく、**弱いモデルのせいで探索、試行錯誤、やり直しが増えないか**を見る。弱いモデルによる長時間の右往左往は、利用枠だけでなく実装時間も浪費する。

設計を済ませてから委任する場合も、設計済みを理由に自動的に弱いモデルへ落とさない。**実装フェーズに残っている判断量**で選ぶ。
