...
- **1 委任 = 1 PR の原則**: 1 回の `a agent new` で委任するタスクは 1 PR 分の作業に限定すること。「複数フェーズを一括で」「複数 PR を順次作成」のような複数 PR をまとめた委任は禁止。委任先は単一の Claude Code プロセスで作業するため、複数 PR を順に作る前提では設計しない。委任先がさらに `delegate` skill で再委任することも想定しない。PR が出ない direct-commit モード (後述) では、この原則を「1 委任 = push される 1 まとまりの変更」と読み替える
- **依存のある PR は委任せず先送りする**: 大きな計画やフェーズ分割されたタスクは、委任元 (現在のセッション) が split-into-prs skill で 1 PR 単位に分割し、単体で merge しても壊れない PR (Batch 1) だけを委任する。複数あれば後述の「複数タスクの一括委任」に従い、1 PR = 1 委任のまま並列に dispatch してよい。先行 PR の merge / 確認を待つ PR は、待ちが解けてから改めて委任元が別途委任する。**依存のある PR を先行 PR と 1 本にまとめ直しても「1 委任 = 1 PR」を満たしたことにはならない**。委任元の裁量で選べる縮退方法は先送りだけで、まとめ直しは入らない。ユーザーが分割の形を指定・承認しているならなおさらで、まとめる方が筋が良いと思ったら後述の「ユーザーが決めたことを委任時に変えない」に従って起動前に諮る。どちらに振り分けるかは番号順ではなく依存順で決まる。計画に「B が無いと A は効かない」に類する記述があれば、番号に関わらず A が後回し

## 絶対禁止事項

- **ユーザーが決めたことを委任時に変えない**: 委任先リポジトリ、PR の分割、各 PR のスコープなど、ユーザーが指定・承認した内容を自分の判断で変えてはならない。ユーザーはどのリポジトリで修正すべきか、どこで PR を切るべきかを把握している。「こっちのリポジトリの方が適切では」「まとめた方が綺麗では」と思ったら、`a agent new` を実行する前に提案して判断を仰ぐ。起動してから事後報告するのは、ユーザーが反対する機会を奪ったことになる
...
- **`a agent peer notify` を進捗確認や催促に使わない**: 委任先は別プロセスだが同じマシン上の Claude Code セッションなので、`a agent peer notify` でメッセージが届く。届くからといって、状況を尋ねる、急かす、作業中に細かく口を出すといった用途に使ってはならない。委任先は対話しながら進める相手ではない。送ってよいのは次の 2 つだけで、いずれも手順は後述の「委任元から委任先に連絡する場合」に従う
...
モデルを 1 段上げると枠の消費は 10 倍になる。
同じモデルで effort を low から max へ上げる消費は 8 倍で、しかも届く品質は後者の方が高い。
同一タスクの実測では luna max が terra low より 3.7 倍安く、既存テストを壊さず、terra が入れた退行も避けていた。

**effort を下げると skill を読みに行かなくなる。**
...
1. `a agent peer children` で対象セッションを特定する。`label` と `cwd` でどの委任か判別し、`session_id` を控える
2. `a agent peer notify --message "<message>" <session_id>` で送る。宛先が `paused` (自動停止済み) でも armyknife が内部で再開してから配送するので、送信前に個別に起こす手順は不要。`ended` (ユーザーが明示的に終了) の場合は再開されずエラーになる
3. 対象が `a agent peer children` に無い、または `notify` がエラーになる場合 (例: `ended` で終了済み) にだけ、委任先が失われたとみなす。PR が作成済みならその PR にコメントを残し、まだ無ければユーザーに報告する
...
**direct-commit モードで作業を終えたら委任元に報告する。** PR が残らないので、報告しない限り委任が終わったことがどこにも伝わらない。通常の委任では成果物の PR が残るため、完了報告は不要。報告に含めるもの:

- どの委任が終わったのか。委任元は複数の委任を並行させているので、タスク名だけでは判別できないことがある
- 成果物の所在 (push したコミット)
...
委任先が委任元に完了報告・訂正・確認を返すときは、`a agent peer parent | jq -r '.[0].session_id // empty'` で委任元の `session_id` を取り、`a agent peer notify --message "<message>" <session_id>` で送る。委任元が `paused` で止まっていても armyknife が内部で再開してから配送する。

`session_id` が空の場合 (`a agent new` 以外で起動されたセッションなど、委任元が記録されていない) や、`notify` が `ended` エラーで失敗する場合は、委任元が失われたとみなす。候補を当てにいかずユーザーに報告する。
