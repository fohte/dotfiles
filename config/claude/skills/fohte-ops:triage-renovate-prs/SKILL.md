...
ユーザーの承認後、各グループの対応を実行する。

### 直接マージの場合

Renovate のブランチで worktree を作成し、ビルド・テストを確認する。**新しいブランチを作らない。Renovate ブランチをそのまま使う。**

```bash
a wm new renovate/<branch-name> --agent --label "<内容>" --prompt "..."
```

確認項目 (言語/フレームワークに応じて選択):

- ビルド (`nr build`, `cargo build` など)
- テスト (`nr test`, `cargo test` など)
- lint (`nr lint`, `cargo clippy` など)

問題なければそのまま merge する。

### 委任の場合 (コード修正が必要 / 複数 PR 統合)
