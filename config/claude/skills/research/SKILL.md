...
**注意**: バグの原因特定や問題の修正が目的の場合は、このスキルではなく `/debug-flow` (Codex: `$debug-flow`) スキルを使用すること。
...
複数の観点で調査が必要な場合は、**必ず複数のサブエージェントを並列起動**する。

- Claude Code: Task ツール。`model: "sonnet"` を指定すること (コスト効率と速度のバランスが良い)
- Codex: `spawn_agent` を並列に呼ぶ (model は省略して親を継承)。結果は `wait_agent` (最大 1 時間) で回収する
