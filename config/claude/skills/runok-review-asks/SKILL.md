...
### 6. ask 維持と決まったコマンドの ignore リスト追記

ステップ 3 で **採用されず「ask 維持」と決着したすべての候補** を、`config/bin/runok-pending-asks` の `IGNORE_COMMANDS` 配列に正規表現として追記する (確認なし)。書き込みは Edit ツールで配列内に 1 行追加。

対象:

- ユーザーが「これは恒久的に無視」と意思表示したもの
- risk profile 上 allow せず ask 維持と判断したもの (例: `cargo run *`)
- 「失敗したコマンドだから提案しない」等の理由で skip したもの

理由: 次回 `runok-pending-asks` で同じ候補が再浮上すると、毎回同じ判断を再現する無駄が発生する。一度 ask 維持と決めた候補は ignore して、次回以降のレビューから除外する。「やはり allow したい」となったときは ignore リストから手動で外せばよい。
