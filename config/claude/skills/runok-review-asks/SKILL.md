...
- **機械的に決まるもの**だけを即振り分け:
    ...
    - 存在しない / 失敗したコマンド → ignore 候補
    - **既存 allow ルールに literal で一致**する重複 → 報告のみ (書き込み対象外)
- main fast-path の判定基準は 3c と同一であり、判定漏れで sub-agent に届いた場合も同じ disposition になる前提。fast-path は overhead 削減のショートカットであり、唯一の正解判定経路ではない
