...
- **9. テスト容易性 (Testability)**: 副作用の分離・mock 可能性・テストの isolation。テスト自体の有無と品質は test-philosophy reviewer が扱うので扱わない。
...
ヘッダは `## 観点別評価 (structure)`。出力では番号付きリストではなく bullet `-` を使い、各行頭に `<番号>.` を手動で書く (prettier の自動再採番を回避するため):
