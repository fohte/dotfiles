...
- 存在しないサブコマンド (`find` / `get` のような直感的な命名からの推測) を実行しないこと。不明な操作は `obsidian help` (詳細は `obsidian help <command>`) で確認してから実行する
- よく使うサブコマンド: `read` (読む、`name=` ではなく `file=`) / `search`・`search:context` (検索) / `create` (新規作成) / `move` (移動、リンク自動更新) / `files` (一覧) / `tag`・`tags` (タグ) / `backlinks`・`links` (被リンク・リンク) / `outline` (見出し一覧)
...
- ノート本文の先頭に H1 見出し (ノートタイトル) を書かない。Obsidian はファイル名をタイトルとして表示するため、本文中のタイトル見出しは冗長になる
